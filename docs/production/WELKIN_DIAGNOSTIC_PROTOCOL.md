...
1. `AGENTS.md` (root) — architectural invariants, distribution model, engineering
   principles.
2. `docs/production/WELKIN_PRODUCTION_READINESS.md` — the production-readiness
   contract (this document's target).
3. `docs/architecture.md`, `docs/deployment.md`, `docs/operator-runbook.md`,
   `docs/implementation.md` — current architecture, deployment, and operator guidance.
4. `certification/catalog.cue` and `docs/certification.md` — guarantees, scenarios,
   and evidence model.
